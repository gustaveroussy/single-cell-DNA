...
if config["type_analysis"] == "dna":
    ...
    fig = sample.heatmap(clusterby='dna', sortby='dna',drop='protein', flatten=False)
    fig.write_html(str_path_result_path+"combined_clusterby_snv_sortby_dna.html")
    
    fig = sample.heatmap(clusterby='dna', sortby='cnv',drop='protein', flatten=False)
    fig.write_html(str_path_result_path+"combined_clusterby_snv_sortby_cnv.html")
    
    fig = sample.heatmap(clusterby='cnv', sortby='cnv',drop='protein', flatten=False)
    fig.write_html(str_path_result_path+"combined_clusterby_cnv_sortby_cnv.html")
    
    fig = sample.heatmap(clusterby='cnv', sortby='dna',drop='protein', flatten=False)
    fig.write_html(str_path_result_path+"combined_clusterby_snv_sortby_cnv.html")
    ...
    fig=sample.dna.scatterplot(attribute="umap",colorby="label")
    fig.write_html(str_path_result_path+"heatmap_dna_colorby_label.html")
    
    fig=sample.cnv.scatterplot(attribute="umap",colorby="label")
    fig.write_html(str_path_result_path+"heatmap_cnv_colorby_label.html")
    ...
    amplicons_to_keep=np.array([sample.cnv.ids()[amp] for amp in 
                   [pval_index_list[i][0] for i in range(len(pval_index_list)) if pval_index_list[i][1] < 0.05]])
    
    fig = sample.dna.heatmap(attribute='AF_MISSING',splitby='label')
    fig.write_html(str_path_result_path+"heatmap__dna_AF_missing_splitby_label.html")
    
    sample.cnv.row_attrs["label_dna"]=sample.dna.row_attrs['label']
    
    fig=sample.cnv.heatmap(attribute='normalized_counts',splitby='label_dna')
    fig.write_html(str_path_result_path+"heatmap_cnv_splitby_dna_label.html")
    ...
    fig.savefig(str_path_result_path+"boxplot_allelic_fraction_clusterby_label.html")
    plt.close(fig)
"""